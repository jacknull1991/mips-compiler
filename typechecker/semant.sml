...
fun actual_ty (ty:T.ty, pos) =
	...
	T.NAME (symb, tyref) =>
	(case !tyref of
             NONE => (error pos ("Undefined actual type: " ^ S.name(symb)); T.NIL)
...
fun lookupType (tenv, ty, pos) =
	...
	NONE => (error pos ("Type undefined: " ^ S.name(ty)); T.NIL)
...
fun transExp (venv, tenv, level, donelabel) =
	...
	fun trexp (A.VarExp var) = trvar var
	  ...
	  | trexp (A.CallExp{func, args, pos}) =
		...
		if (List.length args <> List.length formals) then
			...
			(List.app check_exp_ty arg_forms;
			 {exp = Tr.callExp(level, label, map (#exp) arg_exps), ty = actual_ty(result, pos)})
		    end
	    )
	  | trexp (A.OpExp{left, oper, right, pos}) =
		...
		(case classOp oper of
			...
			| T.STRING => (checkString({exp=rightExp, ty=rightType}, pos);			   
				       {exp = Tr.OpExp(leftExp, oper, rightExp), ty = T.STRING})
			| _ => (error pos "Type mismatch between operands of comparison expression";
				{exp = Tr.OpExp(leftExp, oper, rightExp), ty = T.BUTTOM}))
		   | EqNeqOp =>
		     (case leftType of
			  T.INT => (checkInt({exp=rightExp, ty=rightType}, pos);
				    {exp = Tr.OpExp(leftExp, oper, rightExp), ty = T.INT})
			| T.STRING => (checkString({exp=rightExp, ty=rightType}, pos);
				       {exp = Tr.OpExp(leftExp, oper, rightExp), ty = T.INT})
			(*| T.RECORD _ => (checkRecord({exp=rightExp, ty=rightType}, pos);
					 {exp = Tr.OpExp(leftExp, oper, rightExp), ty = T.INT})
			| T.ARRAY _ => (checkArray({exp=rightExp, ty=rightType}, pos);
					{exp = Tr.OpExp(leftExp, oper, rightExp), ty = T.INT})*)
			| _ => (error pos "Type mismatch between operands of comparison expression";
				{exp = Tr.OpExp(leftExp, oper, rightExp), ty = T.BUTTOM})))
	    end		
		    ...
		    (case t of
			 ...
			 let
				 ...
				 if actual_ty(ty, pos) = #ty (trexp exp) then
					...
					| _ => (error pos ("Undefined field type in record: " ^ S.name symbol); ()))
				 else if #ty (trexp exp) = T.NIL then
				     (case actual_ty(ty, pos) of
					  T.RECORD(fields, unique) => ()
					| _ => (error pos ("Undefined field type in record: " ^ S.name symbol); ()))
				 else
				     (error pos ("Undefined field type in record: " ^ S.name symbol); ())
			 ...
			 in
		       ...
		       | _ => (error pos "Undefined record type";
			       {exp = Tr.nilExp(), ty = T.BUTTOM}))
		end
	    )
	  | trexp (A.SeqExp exps) =
	  ...
	  | trexp (A.AssignExp {var, exp, pos}) =
		...
		if hasSameType(varType, expType, pos, pos) then
		    ...
		    (error pos "Type mismatch between LHS and RHS of assignment";
		      {exp = Tr.assignExp(var', exp'), ty = T.BUTTOM})
	    end
	  ...
	  | trexp (A.IfExp {test, then', else' = SOME else'', pos}) =
		...
		(checkInt(test', pos);
		 if hasSameType(thenType, elseType, pos, pos) then
		     ...
		     (error pos "Type mismatch in THEN and ELSE expressions";
		      {exp = Tr.ifElseExp(#exp test', thenExp, elseExp), ty = T.BUTTOM}))
	    end
	  ...
	  | trexp (A.BreakExp pos) =
	    if (!looplevel = 0) then
		(error pos "Break statement outside of loop"; {exp = Tr.breakExp(donelabel), ty = T.BUTTOM})
	    else if !breakable = false then
		(error pos "Repeated break statement"; {exp = Tr.breakExp(donelabel), ty = T.BUTTOM})
	    else
		     ...
		     (case actual_ty(t, pos) of
			  ...
			  (checkInt(size', pos);
			   if hasSameType(ty, #ty init', pos, pos) then
			       ...
			       (error pos "Type mismatch between expression and array type";
				{exp = Tr.nilExp(), ty = T.BUTTOM}))
	...
	and trvar (A.SimpleVar(id, pos)) =
	    (case S.look(venv, id)
	      of SOME(E.VarEntry {access, ty}) => {exp = Tr.simpleVar(access, level), ty = actual_ty(ty,pos)}
	       | SOME(E.FunEntry _) => (error pos ("Variable expected, but found function");
					{exp = Tr.nilExp(), ty = T.BUTTOM})
	  ...
	  | trvar (A.FieldVar (var, id, pos)) =
		...
		case varType of
		    ...
		    (case List.find (fn (s, t) => s = id) fields of
			 ...
			 in
			     {exp = Tr.arrayVar(varExp, Tr.intExp(fieldIndex)), ty = actual_ty(t, pos)}
			 end
		    )
	  ...
	  | trvar (A.SubscriptVar (var, exp, pos)) =
		...
		case varType of
		    ...
		    in
			case expType of
			    T.INT => {exp = Tr.arrayVar(varExp, expExp), ty = actual_ty(tyArr, pos)}
		...
		(case initType of
		   ...
		   | _ =>
			...
			| SOME (sym, p) =>
			  (case S.look(tenv, sym) of
			       SOME t =>
			       if actual_ty(t, p) = initType then
		  ...
		  | processBody ({name, params, result, body, pos}::decs, venv) =
			...
			fun enterparam ({name, escape, typ, pos}, venv) =
				...
				val ty = case S.look(tenv, typ) of
					     SOME(t) => t
					   | NONE => (error pos ("Undefined parameter type: " ^ S.name(typ));T.BUTTOM)
				val accesslevel = Tr.allocLocal level (!escape)
			    in
