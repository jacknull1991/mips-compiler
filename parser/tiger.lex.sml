...
fun yyAction42 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN COMMENT; netCommentBalance := 1; continue()))
fun yyAction43 (strm, lastMatch : yymatch) = (yystrm := strm;
      (ErrorMsg.error yypos ("closing nonexistent comment!"); continue()))
fun yyAction44 (strm, lastMatch : yymatch) = (yystrm := strm;
      (netCommentBalance := (!netCommentBalance) + 1; continue()))
fun yyAction45 (strm, lastMatch : yymatch) = (yystrm := strm;
      (netCommentBalance := (!netCommentBalance) - 1; if !netCommentBalance = 0 then YYBEGIN INITIAL else (); continue()))
...
fun yyAction49 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN STRING; stringLiteralClosed := false; buffer:= ""; stringStartPosition := yypos; continue()))
fun yyAction50 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm; (buffer := !buffer ^ yytext; continue())
      end
fun yyAction51 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm; (buffer := !buffer ^ (escape yytext); continue())
      end
...
fun yyAction56 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (buffer := !buffer ^ escape (convertControlCharacter(String.substring (yytext, 2, 1))); continue())
      end
fun yyAction57 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm;
        (buffer := !buffer ^ escape (asciiCode(String.substring (yytext, 1, 3))); continue())
      end
fun yyAction58 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN INITIAL; stringLiteralClosed := true; Tokens.STRING(!buffer, !stringStartPosition, yypos)))
...
fun yyAction60 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (ErrorMsg.error yypos ("illegal character within string " ^ yytext); continue())
      end
...
fun yyQ41 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction23(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"*"
              then yyQ115(strm', yyMATCH(strm, yyAction23, yyNO_MATCH))
...
fun yyQ26 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction46(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"*"
              then yyQ27(strm', yyMATCH(strm, yyAction46, yyNO_MATCH))
        ...
        | SOME(inp, strm') =>
            if inp = #"1"
              ...
              then if inp = #"]"
                  ...
                  else yyQ13(strm', yyMATCH(strm, yyAction60, yyNO_MATCH))
            else if inp = #"o"
              then yyAction60(strm, yyNO_MATCH)
            else if inp < #"o"
              then if inp = #"n"
...
fun yyQ21 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction50(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"#"
              then yyQ21(strm', yyMATCH(strm, yyAction50, yyNO_MATCH))
            else if inp < #"#"
              then if inp = #" "
                  then yyQ21(strm', yyMATCH(strm, yyAction50, yyNO_MATCH))
                else if inp < #" "
                  ...
                  then yyAction50(strm, yyNO_MATCH)
                  else yyQ21(strm', yyMATCH(strm, yyAction50, yyNO_MATCH))
            else if inp = #"]"
              then yyQ21(strm', yyMATCH(strm, yyAction50, yyNO_MATCH))
            else if inp < #"]"
              then if inp = #"\\"
