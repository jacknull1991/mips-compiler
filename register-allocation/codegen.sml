...
fun codegen (frame) (stm) : A.instr list =
	...
	fun munchStm (T.SEQ(a, b)) = (munchStm a; munchStm b)
	  ...
	  | munchStm (T.MOVE(T.MEM(T.BINOP(T.PLUS, e1, T.CONST i)), e2)) =
	    emit(A.OPER{assem="sw $rs, " ^ int i ^ "($rt)\n",
	  ...
	  | munchStm (T.MOVE(T.TEMP t, T.CONST i)) =
	    emit(A.OPER{assem="li $rd, " ^ int i ^ "\n",
			src=[],
			dst=[t],
			jump=NONE})
	  (* R(t) <- MEM(e+/-i) *)
	  | munchStm (T.MOVE(T.TEMP t, T.MEM(T.BINOP(T.PLUS, T.CONST i, e)))) =
	    emit(A.OPER{assem="lw $rd, " ^ int i ^ "($rt)\n",
			src=[munchExp e],
			dst=[t],
			jump=NONE})
	  ...
	  | munchStm (T.MOVE(T.TEMP t, T.NAME lable)) =
	    emit(A.OPER{assem="la $rd, " ^ S.name(lable) ^ "\n",
			src=[],
			dst=[t],
			jump=NONE})
	  (* register to register *)		
	  | munchStm (T.MOVE(T.TEMP t, e)) =
	    emit(A.MOVE{assem="move $rd, $rs\n",
			src=munchExp e,
			dst=t})
	  (* jumps *)
	...
	and munchExp (T.MEM(T.BINOP(T.PLUS, e1, T.CONST i))) =
	  ...
	  | munchExp (T.BINOP(binop, e1, e2)) =
	    (result(fn r => emit(A.OPER
				    {assem=binopname(binop) ^ " $rd, $rs1, $rs2\n",
				     src=[munchExp e1, munchExp e2],
				     dst=[r],
				     jump=NONE})))
	  (* register *)		
