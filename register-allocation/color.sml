...
fun color ({interference=Liveness.IGRAPH{graph, tnode, gtemp, moves}, initial, spillCost, registers}) =
	...
	val _ = (TextIO.output(TextIO.stdOut, ("Total number of nodes: " ^ Int.toString(List.length(nodes)) ^ "\n")))		    
	...
	val (precolored, uncolored) = List.partition (fn x => (case T.Table.look(initial, gtemp(x)) of
								   SOME(t) => true
								 | NONE => false)) nodes
	val _ = (TextIO.output(TextIO.stdOut, ("number of uncolored nodes: " ^ Int.toString(List.length(uncolored)) ^ "\n")))
	(* helper functions *)						     
	...
	val _ = (TextIO.output(TextIO.stdOut, ("number of deg < k node: " ^ Int.toString(G.Set.numItems(!simplifyWorklist)) ^ "\n")))
	val spillWorklist = ref (listToSet(List.filter (fn n => numOfAdj(n) >= K) uncolored))
	val _ = (TextIO.output(TextIO.stdOut, ("number of deg >= k node: " ^ Int.toString(G.Set.numItems(!spillWorklist)) ^ "\n")))
		 ...
		 in
		     (removeColor(adjNodes);
		      (if TempSet.isEmpty(!availableColors) = false then
			   ...
			   in
			       (selectStack := l;
				(*TextIO.output(TextIO.stdOut, "Assign color " ^ F.registerName(newcolor) ^ " to node " ^ G.nodename(a) ^ "\n");*)
	...
	repeat();
	assignColors();
	TextIO.output(TextIO.stdOut, "Coloring finished!\n");
	(!colorMap, spills)
    end
