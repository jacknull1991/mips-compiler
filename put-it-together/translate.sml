...
fun StringOpExp (left, oper, right) =
    case oper of
	A.EqOp => Ex(F.externalCall("tig_stringEqual", [unEx left, unEx right]))
      | A.NeqOp => Ex(T.BINOP(T.MINUS, T.CONST 1, F.externalCall("tig_stringEqual", [unEx left, unEx right])))
      | A.LtOp => Ex(F.externalCall("tig_stringLT", [unEx left, unEx right]))
      | A.LeOp => Ex(F.externalCall("tig_stringLE", [unEx left, unEx right]))
      | A.GtOp => Ex(F.externalCall("tig_stringGT", [unEx left, unEx right]))
      | A.GeOp => Ex(F.externalCall("tig_stringGE", [unEx left, unEx right]))
...
fun arrayExp (length, value) =
	...
	val size = T.BINOP(T.PLUS, unEx(length), T.CONST 1)
	val init = unEx(value)
    in
	Ex(T.ESEQ(seq[T.MOVE(T.TEMP arr, F.externalCall("tig_initArray", [size, init])),
		      T.MOVE(T.MEM(T.TEMP arr), unEx(length))],
		  T.BINOP(T.PLUS, T.TEMP arr, T.CONST F.wordsize)))
    end
